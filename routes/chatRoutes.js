...
router.post("/message", async (req, res) => {
  ...
  try {
    let message = await axios.post(`${process.env.CHAT_ROUTE}/addMessage`, {
      chat_id,
      content,
      created_by,
    });
    if (message.status == 201) return res.status(200).json({ message });
  } catch (error) {
    console.log(error);
...
router.post("/chats", (req, res) => {
  ...
  axios
    .post(`${process.env.CHAT_ROUTE}/chats`, {
      user_id,
    })
    .then((msg) => {
...
router.post('/getPrevChat' , (req ,res)=>{
    ...
    axios
      .post(`${process.env.CHAT_ROUTE}/getPrevChat`, {
        chat_id,
      })
      .then((msg) => {
        if (msg.data.error) return res.json({ error: msg.data.error });
